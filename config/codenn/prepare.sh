...
raw_data=../../raw_data/codenn
data_dir=../../data/codenn

rm -rf ${data_dir}
mkdir -p ${data_dir}

../../scripts/prepare-data.py ${raw_data}/train.token nl code ${data_dir} --no-tokenize \
--vocab-size 30000 --shuffle --seed 1234 \
--dev-corpus ${raw_data}/dev.token \
--test-corpus ${raw_data}/test.token \
